...
const ALL_PIECES: [Piece; 7] = [Piece::O, Piece::I, Piece::S, Piece::Z, Piece::T, Piece::L, Piece::J];
...
impl TetrisGame {
    ...
    fn cells(&self) -> [(i32, i32); 4] { Self::cells_for(self.piece, self.rotation) }
    ...
    fn collides(&self, px: i32, py: i32) -> bool { self.collides_shape(px, py, self.cells()) }
    ...
    pub fn is_idle_mode(&self) -> bool { self.idle_mode }
    ...
    fn simulate_drop(
        ...
        while !collides(px, py + 1) {
            ...
            iterations += 1;
            if iterations > 1000 {
                // board should never be this tall
    ...
    fn score_board(board: &[[bool; BOARD_W]], board_h: usize) -> f32 {
        ...
        for col in 0..BOARD_W {
            heights[col] = (0..board_h).find(|&r| board[r][col]).map_or(0, |r| (board_h - r) as i32);
        }
    ...
    fn lock(&mut self) {
        ...
        let full_rows: Vec<usize> = (0..self.board_h).filter(|&r| self.board[r].iter().all(|&c| c)).collect();
