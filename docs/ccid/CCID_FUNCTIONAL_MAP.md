...
Authoritative docs: CCID_PROTOCOL_AND_HIL.md, code_map.md, CCID_TEST_REPORT.md
-->
...
This branch adds a **USB CCID bulk transport** under feature `ccid-openpgp`. There is **no** OpenPGP
...
`xous-log`. `main.rs` does not open PDDB at boot; PDDB provisioning was removed from this PR.
...
| `tools/ccid/ccid_hil/*`, `tools/ccid/ccid_smoke.py` | Host HIL / smoke |
| `tools/ccid/check_ep_budget.py`, `test_ep_budget_cumulative.py`, `sim_persona_a_composite.py` | Host EP / Persona A gates |
| `xtask` | `baosec-ccid`, `ccid-hil` |
| CI workflows | Split to a separate PR (not in this branch tip) |
