...
|------|---------------|-----|
| `tools/ccid/ccid_hil/test_provision.py` | CCID present, **zero CDC**, refuse `--legacy-usb-provision` | Persona A: USB PIN path must stay gone; CDC return = fail |
| `tools/ccid/ccid_hil/ccid_usb.py` | `assert_persona_a_composite` (6 non-EP0, ≥2 HID, ≥1 CCID, 0 CDC) | Catch wrong composite without hardcoded IF indices |
| `tools/ccid/ccid_hil/test_enumerate.py` | CCID descriptor + Persona A (default) | Enum + layout in one gate |
| `tools/ccid/ccid_hil/test_echo.py` | GetSlotStatus / stress XfrBlock via class 0x0B | Transport without OpenPGP handler |
| `tools/ccid/ccid_hil/run_all.sh` | Ordered 00–04; ignores `CCID_HIL_PROVISION` | Full HIL; notes UART OPEN |
| `tools/ccid/ccid_smoke.py` | Enum + Persona A + optional echo | One-shot host sanity |
| `tools/ccid/check_ep_budget.py` | Static EP totals vs 8 | Pre-hardware overflow |
| `tools/ccid/test_ep_budget_cumulative.py` | Independent vs cumulative gap | Documents old guard bug |
| `tools/ccid/sim_persona_a_composite.py` | Mock layouts | No device needed |
